...
この節では **sistatica** を用いて、1要因の反復測定分散分析（repeated-measures ANOVA）を実行する方法を解説します。  
基本的には次の2つの関数を使います。

- `rmOmni()`：オムニバステスト（条件の主効果）
- `rmPost()`：事後検定（対応ありのペア比較：paired t-test）

### 前提：データ形式（long形式）

`rmOmni()` と `rmPost()` は **long形式**のデータフレームを想定します。必要な列は次の3つです。
...
`rmOmni()` は、1要因の反復測定分散分析（rm-ANOVA）の **オムニバステスト（条件の主効果）** を実行します。  
結果は「手法間で共通化された列名」を持つ **1行の tibble** として返されます。
...
球面性補正を常に Greenhouse–Geisser（GG）にしたい場合は `correction = "GG"` を指定します。
...
- `correction`（デフォルト `"auto"`）  
  球面性補正（sphericity correction）の指定（`"auto"`, `"GG"`, `"HF"`, `"none"`）
...
`rmOmni()` の出力は tibble で、主に以下の列を含みます。
...
- `effect_size_ci_low`, `effect_size_ci_high`：効果量CI（現状は予約列で `NA`）

---

## 2) 事後検定：`rmPost()`

`rmPost()` は、条件間のペア比較（対応ありの t 検定）を実行し、  
**多重比較補正後 p 値**、**差の推定量と信頼区間**、**効果量**、さらに **条件ごとの平均・SD・サンプルサイズ** など、論文報告に有用な列をまとめて返します。
...
- `pAdjust`（デフォルト `"holm"`）  
  p値の多重比較補正法。`stats::p.adjust()` が受け付ける文字列（例：`"holm"`, `"bonferroni"`, `"BH"` など）

- `ciAdjust`（`"none"` または `"bonferroni"`）  
  信頼区間の調整法。`"bonferroni"` の場合、比較数に基づいて CI の信頼水準を調整します。

### 出力（`post`）の列の意味（要点）

`rmPost()` は **ペア比較ごとに1行**の tibble を返します。主な列は以下です。
...
- `ci_adjustment`：CI調整の有無（`"unadjusted"` / `"bonferroni_adjusted"`）
