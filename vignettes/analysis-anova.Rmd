---
title: "sistatica: 4種類の一要因デザインに対するオムニバス検定と事後検定"
output:
  rmarkdown::html_vignette:
    toc: true
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{sistatica: 4種類の一要因デザインに対するオムニバス検定と事後検定}
...
このビネットは、**sistatica** パッケージで提供される 4 種類の一要因デザインについて、**オムニバス検定**と**事後検定（ペア比較）**を実行する際の、パッケージ固有の仕様・引数・出力の読み方をまとめます。

## 0.1 関数一覧

| 手法 | オムニバス検定 | 事後検定 |
|---|---|---|
| パラメトリック・対応あり（反復測定） | `rm_anova_omnibus()` | `rm_posthoc()` |
| パラメトリック・対応なし（Welch） | `welch_anova_omnibus()` | `welch_posthoc()` |
| 非パラメトリック・対応あり（Friedman） | `friedman_omnibus()` | `friedman_posthoc()` |
| 非パラメトリック・対応なし（Kruskal-Wallis） | `kruskal_omnibus()` | `kruskal_posthoc()` |
...
- CI補正: `ci_adjust_method`（推奨例: `"bonferroni"`、選択肢は `"none"`, `"bonferroni"`, `"sidak"`）

この独立性のため、例えば  
`p_adj < 0.05`（有意）だが `ci_low_adj`〜`ci_high_adj` が 0 を含む、のような **不一致**が起こりえます。これはバグではなく、異なる誤差制御戦略を併用した帰結です。
...
- 反復測定 / Friedman（対応あり）
  - `"pairwise"`: 行単位で `dv/within/id` の欠損を除いた後、効果量とCIは **各ペアで両条件が揃うID**のみを用いて計算します。
  - `"complete_case"`: 全条件にわたって欠損のないID（完全ブロック）のみで分析します。
- Welch / Kruskal-Wallis（対応なし）
  - 実質的にどちらも「欠損行を落とす」挙動になりやすいですが、APIとして引数は残しています。

## 0.6 重複セル（ID×within）の扱い（対応ありの事後検定）

対応あり（`rm_posthoc()`, `friedman_posthoc()`）では、`id × within` セルが重複するデータに対し、次の選択肢があります。

- `duplicate = "mean"`（デフォルト）: 各参加者（id）が各条件（within）で複数回測定されているデータは、いったん「参加者×条件」に1値へ集約（平均値を使用）してから、通常の一要因・反復測定として解析されます。
- `duplicate = median" / "first"`: 中央値で集約する、あるいはデータ内で最初に登場する値を使う。
- `duplicate = "error"`（デフォルト）: 重複があれば停止

`duplicate = "first"` かつ `duplicate_na_rm = TRUE` の場合は、重複セル内の NA を除いてから最初の値（first non-NA）を取ります。
...
- オムニバス: `rm_anova_omnibus()`（反復測定ANOVA）
  - 効果量: **GES（generalized eta-squared）**
- 事後: `rm_posthoc()`（対応のある t 検定）
  - 効果量: **dz（Cohen’s d for paired / repeated measures）**
  - 効果量CI: `effectsize::repeated_measures_d(method="z")` による解析的CI
  - p値補正 / CI補正は独立（イントロ参照）

## 1.2 データ（ロング形式の既存データセット）

ここでは `datasets::CO2` を用います（ロング形式、`Plant` がID、`conc` が within）。
...
```{r rm-omnibus-inspect, eval=FALSE}
...
- CI:
  ...
  - 補正: `ci_low_adj`, `ci_high_adj`（`ci_level_adj`, `ci_adjust_method`）
- `n_pair` はそのペアで効果量・CIに使えた対応データ数

（確認用の操作例：本ビネットでは実行しません）
```{r rm-posthoc-inspect, eval=FALSE}
rm_post %>%
...
- オムニバス: `welch_anova_omnibus()`（Welch一要因ANOVA）
  - 効果量: **omega²**（WelchのFと自由度から計算した近似値）
- 事後: `welch_posthoc()`（Welch型のペア比較：`pool.sd = FALSE`）
...
ここでは `datasets::PlantGrowth` を用います（ロング形式）。
...
- 効果量: `effect_size_type = "omega2"` と `effect_size`
  - Welch ANOVA に対する omega² は便宜的に提供される**近似**であり、厳密な等分散前提のomega²と同一視しない（パッケージの注記）。

（確認用の操作例：本ビネットでは実行しません）
```{r welch-omnibus-inspect, eval=FALSE}
...
（確認用の操作例：本ビネットでは実行しません）
```{r welch-posthoc-inspect, eval=FALSE}
welch_post %>%
...
反復測定と同様に `datasets::CO2` を用い、within = `conc`, id = `Plant` とします。
...
（確認用の操作例：本ビネットでは実行しません）
```{r friedman-omnibus-inspect, eval=FALSE}
...
（確認用の操作例：本ビネットでは実行しません）
```{r friedman-posthoc-inspect, eval=FALSE}
fr_post %>%
  dplyr::select(group1, group2, p_adj, p_adj_mark, effect_size, ci_low, ci_high, ci_low_adj, ci_high_adj, n_pair) %>%
  dplyr::arrange(p_adj)
```
...
Welchと同様に `datasets::PlantGrowth` を用います。
...
（確認用の操作例：本ビネットでは実行しません）
```{r kruskal-omnibus-inspect, eval=FALSE}
...
（確認用の操作例：本ビネットでは実行しません）
```{r kruskal-posthoc-inspect, eval=FALSE}
kw_post %>%
  dplyr::select(group1, group2, p_adj, p_adj_mark, effect_size, ci_low, ci_high, ci_low_adj, ci_high_adj) %>%
  dplyr::arrange(p_adj)
```
